...
@router.post("/")
async def add_new_origin(name: str, path: str):
    """Saves an origin folder to database."""
    # OK
    try:
        async with async_session() as session:
            result = await base_route.add_new(session, Origin, name=name, path=path)
            return {"code": 200,
                    "message": "Origin added successfully",
                    "id": result.id, "path": result.path}
    except IntegrityError as e:
        handle_db_session_exception(e, status_code=409)
    except SQLAlchemyError as e:
        handle_db_session_exception(e)


@router.get("/{origin_id:int}/items")
async def get_origin_items(origin_id: int):
...
@router.get("/{origin_name}")
async def get_origin_by_name(origin_name: str):
    try:
        async with async_session() as session:
            async with session.begin():
                return await base_route.get_by_name(session=session, model=Origin, name=origin_name)
    except SQLAlchemyError as e:
        handle_db_session_exception(e)
