...
class FormatControl(ft.UserControl):
    ...
    def validate_format_name(self, format_name):
    ...
    async def open_alert(self, alert: ft.AlertDialog):
        self.page.dialog = alert
        alert.open = True
        await self.page.update_async()

    async def close_alert(self, e, alert: ft.AlertDialog):
        alert.open = False
        await self.page.update_async()
    ...
    async def submit_format_row(self, e):
        ...
        try:
            if self.format_name_field.value and self.target_dropdown.value:
                if not self.validate_format_name(self.format_name_field.value):
                    raise InvalidEntryException(message="format name is invalid.")
            ...
            async def close_button_click(e):
                asyncio.create_task(self.close_alert(e, alert=alert))
